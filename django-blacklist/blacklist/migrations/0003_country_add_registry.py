...
class Migration(DataMigration):
    def forwards(self, orm):
		self.country_to_rir = {}
		for line in open("blacklist/migrations/data/country-to-rir.txt", "r").readlines():
			line = line.strip()
			(name, code, forget, rir) = line.split("\t")
			self.country_to_rir[code] = rir

		for country in orm.Country.objects.all():
			country.rir = orm.RIR.objects.get(
				name=self.country_to_rir[country.code]
			)
			country.save()
